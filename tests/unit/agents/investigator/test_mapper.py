...
from sentinel.agents.planner.models import PlanStep, PlanStepType
...
@pytest.mark.parametrize(
    ...
    [
        (
            PlanStepType.INSPECT_FILE,
            {"path": "src/service.py"},
            "read_file",
        ),
        (
            PlanStepType.SEARCH_CODE,
            {"query": "validate_token"},
            "search_code",
        ),
        (
            PlanStepType.RUN_TESTS,
            {"test_path": "tests"},
            "run_tests",
        ),
        (
            PlanStepType.ANALYZE_LOGS,
